...
***抽象字符***(Abstract Character)：用于组织、控制或表示文本数据的信息单元。(See definition D7 in [Section 3.4, Characters and Encoding](http://www.unicode.org/versions/latest/ch03.pdf#G2212).)。一个抽象字符可以对应多个码点，比如 "Å" 被映射到 U+00C5 和 U+212B。
...
***已编码字符*** (Encoded Character)：抽象字符(abstract character)和码点(code point)之间的关联(或映射)[See definition D11 in [Section 3.4, Characters and Encoding](http://www.unicode.org/versions/latest/ch03.pdf#G2212).]。抽象字符本身没有数值值，但"encoding a character"的步骤会将特定的码点与特定的抽象字符关联。另外，Encoded Character 与 Coded Character 同义。已编码字符中不包含*未分配字符*。有时，为了与其他标准兼容，一个抽象字符可以对应多个码点，比如 "Å" 被映射到 U+00C5 和 U+212B。
