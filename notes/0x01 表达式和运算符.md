...
在 `a op1 b op2 c` 中，如果 `a op1 b` 为假，
...
用户定义(User-defined)对象可通过 [`__bool__()`](https://docs.python.org/3/reference/datamodel.html#object.__bool__) 方法定义真值。

- `or` (boolean OR)

  对于表达式 `x or y`，首先会计算 *x* 的值，如果该值的布尔意义为真，则返回该值；如果该值的布尔意义为假，才会计算并返回 *y* 的值。
...
- `and` (boolean AND) 
  ...
  对于表达式 `x and y`，首先会计算 *x* 的值，如果该值的布尔意义为假，则返回该值；如果该值的布尔意义为真，才会计算并返回 *y* 的值。
