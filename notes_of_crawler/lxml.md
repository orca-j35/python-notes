...
lxml.etree 还提供了 XPath 评估器(*evaluator*)的类: lxml.etree.[XPath](https://lxml.de/api/lxml.etree.XPath-class.html) 和 lxml.etree.[XPathEvaluator](https://lxml.de/api/lxml.etree-module.html#XPathEvaluator)。
...
在 ElementTree 对象上调用 `xpath()` 方法时，将针对文档(如果是绝对路径)或根节点(如果是相对路径)执行全局 XPath 查询:
